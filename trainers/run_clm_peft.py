...
def main():
    ...
    with training_args.main_process_first(desc="Preparing directory"):
        huggingface_hub.login() #login to huggingface hub with the token
    ...
    def tokenize_function(examples):
        if check_eos_ending(): #meaning that EOS token is not added by tokenizer/ some tokenizer is broken!!!!
            EOS_TOKEN = tokenizer.eos_token # this is the token that will be used to separate the texts
            examples[text_column_name] = [
                text + EOS_TOKEN for text in examples[text_column_name]
            ]
    ...
    if lm_datasets is None:
        ...
        if data_args.save_to_disk and data_args.data_cache_dir is not None:
                ...
                lm_datasets.push_to_hub(repo_id=data_args.hub_id, max_shard_size="2GB")
    ...
    if training_args.do_train:
        ...
        try:
            ...
            #This is for preparing fsdp saved
